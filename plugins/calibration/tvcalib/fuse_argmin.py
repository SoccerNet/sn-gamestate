...
for subset_dir in subset_dirs:
    ...
    df_subset["subset"] = file_per_sample_output.parent.parent.name
    df_subset.drop(
        columns=[
            "batch_idx",
            "time_s",
...
#%%
# select candidate from loss_ndc_total.min()
# df = df.loc[df.groupby("image_ids")["loss_ndc_total"].idxmin()] # not working?
df = df.sort_values(by="loss_ndc_total").groupby("image_ids").apply(pd.DataFrame.head, n=1)
df.set_index("image_id", inplace=True)
print(df)
print(df.subset.value_counts())
#%%
fout = args.result_dir_base / args.subset_glob.replace("?", "argmin") / "per_sample_output.json"
print(fout)
